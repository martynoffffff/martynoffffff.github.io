...
if ($data->type == 'message_new') { // Если это новое сообщение то выполняем код указанный в условии
 ...
 if($peer_id == 2000000002 or $peer_id == 2000000001){
  ...
  // $get_members = $vk->request('messages.getConversationMembers', ['peer_id' => $peer_id]); // Получили список пользователей беседы
   ...
   //   $user_id = $member['id']; // Получили id пользоавтеля
        ...
        // if (in_array($id, $is_admin)) { 
            ...
            $vk->sendMessage($peer_id, "Объявление - {$obvl} - @id189374560 (Поля) @id335635632 (Ксения) @id467722323 (Полина) @id401756219 (Александра) @danidze @id333153540 (Таисия) @golubicka @lzbrdlnnn @id365688934 (Ульяна) @levsuprun @id491263690 (Зоя) @id491263372 (Арина) @id400322865 (Артём) @id323269958 (Ксюша) @id211340215 (Александр) @id170059816 (Юлия) @nefariushd @soniamish @id222565521 (Вероника) @id482483130 (Лиза) @bobulyaka @id153497583 (Григорий) @ivan_galaev @id546582813 (Екатерина) @polarizasion1 @id397444469 (Даниил) ");
...
//Седьмой урок(14:45-15:35)");
 //      }
     ...
     $vk->sendMessage($peer_id, "Первый урок(9.00-9.30)
Второй урок(9.40-10.10)
Третий урок(10.20-10.50)
Четвертый урок(11.00-11.30)
Пятый урок(11.40-12.10)
Шестой урок(12.20-12.50)
Седьмой урок(13.00-13.30)");
}
///////////////////////////////////////// 
...
/////////////////////////////////////////        
 ...
 if ($message == '/чат') {
           $vk->sendMessage($peer_id, "Айди данного чата - {$peer_id}");
}       
        
   
 //if ($message == "pin") {
 //         $vk->request('messages.pin', ['peer_id' => $peer_id, 'message_id' => $reply_message]);
//}       
     
        
 // if ($message == 'пх'){
   // $gg = rand(1,2);
    // $userInfo = $vk->request("users.get", "first_name");
  // $userInfo = $vk->request("users.get", ["user_ids" => $reply_author]);
  //$first_name = $userInfo[0]['first_name'];
 //  
...
//} 
  ...
  //  if ($gg == 1){
   //$vk->sendMessage($peer_id, "$reply_author посещал(а) PornHub.");
  //  }else{

  //  $vk->sendMessage($peer_id, "$reply_author не посещал(а) PornHub.");
...
///////////////////////////////////////// не готовое /////////////////////////////////////////        
    ...
    if( mb_substr($message,0,6)=='погода'){
...
$city =  substr($message,13); // Объединили текст после команды в единый
...
$OWApi_key = 'a3d9811bdd3e66cd6fe622b335bf4304'; // Ваш ключ от OpenWeatherMap
...
$vk->sendMessage($peer_id, "✖ Ой.. Прости я не понял, что это за город такой 😿");
...
$vk->sendMessage($peer_id, "Ошибка: ".$e->getMessage() );
     ...
     $vk->sendMessage($peer_id, "Чат не является исполняемым.");
 } 
